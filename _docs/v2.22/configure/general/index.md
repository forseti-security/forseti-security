...
* [`inventory`]({% link _docs/v2.22/configure/inventory/index.md %}): configurations that are used
only by Forseti Inventory, such as
  api quota usage and inventory purge option.
* [`scanner`]({% link _docs/v2.22/configure/scanner/index.md %}): configurations that are used
only by Forseti Scanner, such as
    specifying which scanners to enable.
* [`notifier`]({% link _docs/v2.22/configure/notifier/index.md %}): configurations that are used
...
Learn how to manually [reload the server configuration]({% link _docs/v2.22/use/cli/server.md %}).
