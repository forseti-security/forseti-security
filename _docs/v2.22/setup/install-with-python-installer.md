...
### Run setup
  ...
  1. Install both client and server by running the installer:
     ...
     ```
     You can read more about the composite root resources [here]({% link _docs/v2.22/configure/general/non-org-root.md %}).
  ...
  1. The installer will infer the necessary information to install Forseti.
     ...
     * SendGrid API key \[Optional\]: Used for sending email via SendGrid. For
       ...
       [enable email notifications]({% link _docs/v2.22/configure/notifier/index.md %}#email-notifications).
