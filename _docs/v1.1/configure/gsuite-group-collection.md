...
[next section]({% link _docs/v1.1/configure/gsuite-group-collection.md %}#enable-the-service-account-in-your-g-suite-admin-control-panel).
...
1. On the **Create service account** dialog that appears, set up your service account:
   ...
   * If you haven't already configured your project's OAuth consent screen, enter a product name
      ...
      ![create service account window with product name field highlighted](/images/docs/v1.1/configuration/create-service-account.png)
        
1. To create and download a JSON key for the service account:

   * Click **More** on the service account row, then click **Create key**.
      ![more menu with create key highlighted](/images/docs/v1.1/configuration/create-key.png)
...
1. On the **Client ID for Service account client** panel that appears, copy the **Client ID**
   ...
   ![service account panel with client ID highlighted](/images/docs/v1.1/configuration/client-id.png)
...
1. In the **One or More API Scopes** box, paste the following scope:
    ...
    ```
1. Click **Authorize**.
   ![manage api client access in Google Admin Security settings](/images/docs/v1.1/configuration/admin-security.png)
...
["Move Configuration to GCS"]({% link _docs/v1.1/setup/gcp-deployment.md %}#move-configuration-to-gcs) 
for details on how to do this.

## Deploying to GCP with G Suite Google Groups collection

If you
[created a deployment]({% link _docs/v1.1/setup/gcp-deployment.md %})
