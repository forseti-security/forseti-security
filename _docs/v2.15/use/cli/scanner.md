...
[define custom rules]({% link _docs/v2.15/configure/scanner/rules.md %}).
...
If you haven't created a model yet, see [create a model]({% link _docs/v2.15/use/cli/model.md %}).

To configure which scanners to run, see
[Configuring Forseti: Configuring Scanner]({% link _docs/v2.15/configure/scanner/index.md %}).
...
* Learn how to [use Notifier]({% link _docs/v2.15/use/cli/notifier.md %}) to send notifications on the
