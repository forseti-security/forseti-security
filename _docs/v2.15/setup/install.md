...
### Run setup
  ...
  1. Install both client and server by running the installer:
     ...
     ```
     You can read more about the composite root resources [here]({% link _docs/v2.15/configure/general/non-org-root.md %}).
  ...
  1. The installer will infer the necessary information to install Forseti.
     ...
     * SendGrid API key \[Optional\]: Used for sending email via SendGrid. For
       ...
       [enable email notifications]({% link _docs/v2.15/configure/notifier/index.md %}#email-notifications).
     ...
     * G Suite super admin email \[Optional\]: This is part of the
       [G Suite data collection]({% link _docs/v2.15/configure/inventory/gsuite.md %}).
...
* Learn how to customize
  [Inventory]({% link _docs/v2.15/configure/inventory/index.md %}) and
...
* Learn how to [configure Forseti to run from non-organization
  root]({% link _docs/v2.15/configure/general/non-org-root.md %}).
