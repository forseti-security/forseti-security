...
* `name`
  ...
  * **Valid values**: The name must match the actual module name for each notifier in
  [forseti-security/google/cloud/forseti/notifier/notifiers]({% link _docs/v2.20/develop/reference/google.cloud.forseti.notifier.notifiers.html %}),
...
#### Prerequisites
1. [Install]({% link _docs/v2.20/setup/install.md %})
or [upgrade]({% link _docs/v2.20/setup/upgrade.md %}) Forseti to version 2.14+. 
...
To verify violations appear in the Cloud SCC Dashboard, [run the notifier]({% link _docs/v2.20/use/cli/notifier.md %})
after you have [built an inventory]({% link _docs/v2.20/use/cli/inventory.md %})
and [run the scanner]({% link _docs/v2.20/use/cli/scanner.md %}).
...
* Learn how to [invoke Forseti Notifier]({% link _docs/v2.20/use/cli/notifier.md %}).
