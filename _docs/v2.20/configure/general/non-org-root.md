...
Run the Forseti [Installer]({% link _docs/v2.20/setup/install.md %}).
...
1. **NEW for version 2.12.0+**: You can use the `composite_root_resources`
   ...
   See [Configure Inventory]({% link _docs/v2.20/configure/inventory/index.md %})
   for more details.
...
1. If Foresti was not installed with Org Admin credentails, then you need
   ...
   organization]({% link _docs/v2.20/concepts/service-accounts.md %}#the-server-service-account).

1. Saving changes.
   1. Save the changes to `forseti_conf_server.yaml` file.
   1. Upload `forseti_conf_server.yaml` [to GCS bucket]({% link _docs/v2.20/configure/general/index.md %}#moving-configuration-to-cloud-storage).
...
1. Use the updated configuration.
   ...
   1. Make the server [reload the updated configuration]({% link _docs/v2.20/use/cli/server.md %}).
