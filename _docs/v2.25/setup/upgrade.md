...
If your Forseti deployment was previously deployed with Deployment Manager, please see the [migration documentation]({% link _docs/v2.25/setup/migrate.md %}) on migrating to Terraform.  Following these steps will also result in an upgraded deployment of Forseti.
...
1. Update the `version` inside `main.tf` file to `5.2.2`.
1. Run command `terraform init` to initialize terraform.
1. Run command `terraform plan` to see the infrastructure plan.
1. Run command `terraform apply` to apply the infrastructure build.
...
{% endcapture %}
{% include site/zippy/item.html title="Upgrading 2.24.0 to 2.25.0" content=upgrading_2_24_0_to_2_25_0 uid=26 %}
...
* Customize [Inventory]({% link _docs/v2.25/configure/inventory/index.md %}) and
...
* Enable [G Suite data collection]({% link _docs/v2.25/configure/inventory/gsuite.md %})
