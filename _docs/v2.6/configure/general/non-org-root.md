...
Run the Forseti [Installer]({% link _docs/v2.6/setup/install.md %}).
...
organization]({% link _docs/v2.6/concepts/service-accounts.md %}#the-server-service-account).

1. Saving changes.
   1. Save the changes to `forseti_conf_server.yaml` file.
   1. Upload `forseti_conf_server.yaml` [to GCS bucket]({% link _docs/v2.6/configure/general/index.md %}#moving-configuration-to-cloud-storage).
...
1. Use the updated configuration.
   ...
   1. Make the server [reload the updated configuration]({% link _docs/v2.6/use/cli/server.md %}).
