...
class GceNetworksRulesEngine(bre.BaseRulesEngine):
...
#TODO rename enforced_networks_rules?
    def find_policy_violations(self, network_interface):
        ...
        if self.rules.project != '^.+$':
            project_name_bool = re.match(self.rules.project, network_interface.project)

        if self.rules.network != '^.+$':
            network_name_bool = re.match(self.rules.network,
                                          network_interface.network)

        if self.rules.is_external_network != '^.+$':
            is_external_network_bool = re.match(self.rules.is_external_network,
