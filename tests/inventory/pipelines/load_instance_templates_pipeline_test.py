...
class LoadInstanceTemplatesPipelineTest(basetest.TestCase):
    ...
    def test_can_transform_instance_templates(self):
        """Test transform function works."""
        actual = self.pipeline._transform(
            fake_instance_templates.FAKE_PROJECT_INSTANCE_TEMPLATES_MAP)
        self.assertEquals(
    ...
    def test_retrieve_data_is_correct(
        ...
        self.pipeline.api_client.get_instance_templates = mock.MagicMock(
            side_effect=[fake_instance_templates.FAKE_API_RESPONSE1,
                         fake_instance_templates.FAKE_API_RESPONSE2])
    ...
    def test_retrieve_error_logged_when_api_error(
        ...
        mock_get_projects.return_value = self.projects
        self.pipeline.api_client.get_instance_templates.side_effect = (
            api_errors.ApiExecutionError(self.resource_name, mock.MagicMock()))
        load_instance_templates_pipeline.LOGGER = mock.MagicMock()
        self.pipeline._retrieve()

        self.assertEqual(
            len(self.project_ids),
            load_instance_templates_pipeline.LOGGER.error.call_count)
    ...
    def test_pipeline_no_rules_loads_nothing(
        ...
        self.pipeline.dao.select_record_count = mock.MagicMock(
            side_effect=data_access_errors.MySQLError(
                'instance_templates', mock.MagicMock()))
