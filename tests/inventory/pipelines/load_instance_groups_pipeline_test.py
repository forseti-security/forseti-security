...
from tests.inventory.pipelines.test_data import fake_instance_groups
...
class LoadInstanceGroupsPipelineTest(basetest.TestCase):
    ...
    def test_can_transform_instance_groups(self):
        """Test transform function works."""
        actual = self.pipeline._transform(
            fake_instance_groups.FAKE_PROJECT_INSTANCE_GROUPS_MAP)
        self.assertEquals(
    ...
    def test_api_is_called_to_retrieve_instance_groups(
            self, mock_get_projects, mock_conn):
        """Test that API is called to retrieve instance_groups."""
        mock_get_projects.return_value = self.projects
        self.pipeline._retrieve()
        self.assertEqual(
            len(self.project_ids),
            self.pipeline.api_client.get_instance_groups.call_count)
    ...
    def test_retrieve_data_is_correct(
        ...
        self.pipeline.api_client.get_instance_groups = mock.MagicMock(
            side_effect=[fake_instance_groups.FAKE_API_RESPONSE1,
                         fake_instance_groups.FAKE_API_RESPONSE2])
    ...
    def test_retrieve_error_logged_when_api_error(
        ...
        mock_get_projects.return_value = self.projects
        self.pipeline.api_client.get_instance_groups.side_effect = (
            api_errors.ApiExecutionError(self.resource_name, mock.MagicMock()))
        load_instance_groups_pipeline.LOGGER = mock.MagicMock()
        self.pipeline._retrieve()

        self.assertEqual(
            len(self.project_ids),
            load_instance_groups_pipeline.LOGGER.error.call_count)
