...
class TestExplainerAccessByMember:
    ...
    def test_access_by_member(self, forseti_cli, forseti_model_readonly,
    ...
    @pytest.mark.e2e
    @pytest.mark.explainer
    def test_access_by_member_with_permission(self, forseti_cli,
        ...
        # Act
        result = forseti_cli.explainer_access_by_member(
            f'serviceaccount/{forseti_server_service_account}', 'storage.buckets.list')

        # Assert Server SA has permission granted from roles/iam.securityReviewer
        assert re.search(fr'"resources":[\s+]\[\\n'
                         fr'[\s+]*"organization\/{organization_id}"\\n'
                         fr'[\s+]*\],\\n'
                         fr'[\s+]*"role":[\s+]"roles\/iam.securityReviewer"',
